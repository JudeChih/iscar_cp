...
<!--车款分类 end-->
<div id="body">
    <h2><?=$lang_carinfo['汽车信息编辑']?></h2>
    <form id="form" action="<?=URL?>iscaradmin/?p=carinfo&a=edit&id=<?=$post['ci_id']?>" method="post" enctype="multipart/form-data" onsubmit="return verify_order()">
        ...
        <table width="100%" style="line-height: 18px;margin-top: 20px;">
            ...
            <tr>
                <td>
                    <input type="text" class="inline form-control" disabled="disabled" id="carstyles_v" style="width: 580px;" value="<?=$post['cs_fullname']?>">
                </td>
                <input type="hidden" id="carstyles" name="ci_model_style" value="<?=$post['ci_model_style']?>">
            </tr>
        </table>
...
<!--                <td><label>--><?//=$lang_carinfo['车部件安装位置']?><!--:</label></td>-->
            ...
            <tr>
                ...
                <td><input type="text" class="inline form-control " name="ci_fuel_tax" maxlength="8"   onblur="validate_num(this)"  onchange="validate_submit(this)"  style="width: 150px;" value="<?=$post['ci_fuel_tax']?>"></td>
                <td><input type="text" class="inline form-control " name="ci_license_tax" maxlength="8"  onblur="validate_num(this)"  onchange="validate_submit(this)"  style="width: 150px;" value="<?=$post['ci_license_tax']?>"></td>
<!--                <td>-->
                ...
                <td>
                    <select class="form-control" style="width: 150px" name="ci_region" required>
                        ...
                        <?php foreach($terms_re as $v):?>
                            <option value="<?=$v['keyword']?>" <?=$post['ci_region']==$v['keyword']?'selected':''?>><?=$v['value']?></option>
                        <?php endforeach;?>
                    </select>
                </td>
            </tr>
