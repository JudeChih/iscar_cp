...
<!--厂牌名称 end-->
<div id="body">
    <h2><?=$lang_carmodels['汽车车系编辑']?></h2>
    <form action="<?=URL?>iscaradmin/?p=carmodels&a=edit&id=<?=$post['cm_id']?>" method="post" enctype="multipart/form-data" onsubmit="return verify()">
        ...
        <div>
            <div style="width: 500px">
                <label><?=$lang_carmodels['车系名称']?>:</label><br>
                <input type="text" class="inline form-control " name="cm_fullname"  value="<?=$post['cm_fullname']?>" required>
            </div>
            <div style="width: 300px">
                <label><?=$lang_carmodels['车系昵称']?>:</label>
                <input type="text" class="inline form-control " name="cm_nickname"  value="<?=$post['cm_nickname']?>" >
            </div>
        </div>
        <div>
            <div style="width: 200px">
                <label><?=$lang_carmodels['缩写']?>:</label><br>
                <input type="text" class="inline form-control " name="cm_short_name" value="<?=$post['cm_short_name']?>" >
            </div>
<!--            <div style="width: 100px">-->
<!--                <label>--><?//=$lang_carmodels['序号']?><!--:</label>-->
<!--                <input type="text" class="inline form-control " id="list" name="cm_list_order" value="--><?//=$post['cm_list_order']?><!--" onblur="validate_num(this)" maxlength="8">-->
<!--            </div>-->
        </div>
                ...
                <td>
                    <select class="form-control" style="width: 150px" name="cm_region" required>
                        ...
                        <?php foreach($terms_re as $v):?>
                            <option value="<?=$v['keyword']?>" <?=$post['cm_region']==$v['keyword']?'selected':''?>><?=$v['value']?></option>
                        <?php endforeach;?>
                    </select>
                </td>
            </tr>
        ...
        <!--<div>
            ...
            <div class="file-box">
                <input type="text" class="txt" id="textfield" value="<?/*=$post['cm_icon_path']*/?>" >
                ...
                <br><img style="max-width: 100px;max-height: 100px" src="./<?/*=$post['cm_icon_path']*/?>" class="img-rounded">
            </div>
        </div>-->
        <div>
            <label><?=$lang_carmodels['热门标示']?>:</label><br>
            <span class="radio-inline">
                <input type="radio" name="cm_hot_item_tag" value="3" <?=$post['cm_hot_item_tag']==3 ? 'checked' : ''?>> <?=$lang_carmodels['是']?>
            </span>
            <span class="radio-inline">
                <input type="radio" name="cm_hot_item_tag" value="1" <?=$post['cm_hot_item_tag']==1 ? 'checked' : ''?>> <?=$lang_carmodels['否']?>
            </span>
    ...
    function verify() {
        ...
        if (!$.trim(cb)){
            ...
            $.ajax({
                ...
                type: 'post',
                async: false,
                data: {'cm_name': cm_name, 'a': 'verify_cm_name', 'cb_id' : cb_id, 'id' : id},
                dataType: 'json',
                error: function () {
